...
var EndpointGenerator = /** @class */ (function () {
    function EndpointGenerator(node, name) {
        this.node = node;
        this.debugger = debug("typescript-rest-swagger:metadata:".concat(name));
    }
    EndpointGenerator.prototype.getDecoratorValues = function (decoratorName, acceptMultiple) {
        ...
        var decorators = (0, decoratorUtils_1.getDecorators)(this.node, function (decorator) { return decorator.text === decoratorName; });
        if (!decorators || !decorators.length) {
            return [];
        }
        if (!acceptMultiple && decorators.length > 1) {
            throw new Error("Only one ".concat(decoratorName, " decorator allowed in ").concat(this.getCurrentLocation(), "."));
        }
        var result;
        if (acceptMultiple) {
    ...
    EndpointGenerator.prototype.getResponses = function (genericTypeMap) {
        var _this = this;
        var decorators = (0, decoratorUtils_1.getDecorators)(this.node, function (decorator) { return decorator.text === 'Response'; });
        if (!decorators || !decorators.length) {
            return [];
        }
