...
function resolveType(typeNode, genericTypeMap) {
    ...
    if (typeNode.kind !== ts.SyntaxKind.TypeReference) {
        throw new Error("Unknown type: ".concat(ts.SyntaxKind[typeNode.kind]));
    }
...
function getPrimitiveType(typeNode) {
    ...
    if (primitiveType === 'number') {
        ...
        var jsdocTagName = (0, jsDocUtils_1.getFirstMatchingJSDocTagName)(parentNode, function (tag) {
            return validDecorators_1.some(function (t) { return t === tag.tagName.text; });
        });
        var decoratorName = (0, decoratorUtils_1.getDecoratorName)(parentNode, function (identifier) {
...
function getEnumerateType(typeNode) {
    ...
    if (enumTypes.length > 1) {
        throw new Error("Multiple matching enum found for enum ".concat(enumName, "; please make enum names unique."));
    }
...
function getLiteralType(typeNode) {
    ...
    if (literalTypes.length > 1) {
        throw new Error("Multiple matching enum found for enum ".concat(literalName, "; please make enum names unique."));
    }
...
function getReferenceType(type, genericTypeMap, genericTypes) {
    ...
    try {
        ...
        console.error("There was a problem resolving type of '".concat(getTypeName(typeName, genericTypes), "'."));
        throw err;
    }
}
...
function getModelTypeDeclaration(type) {
    ...
    if (!modelTypes.length) {
        throw new Error("No matching model found for referenced type ".concat(typeName));
    }
    ...
    if (node.kind === ts.SyntaxKind.InterfaceDeclaration) {
        ...
        return interfaceDeclaration.members
            ...
            if (indexType.typeName !== 'string') {
                throw new Error("Only string indexers are supported. Found ".concat(indexType.typeName, "."));
            }
            return {
                description: '',
                name: '',
...
function getModifiers(node) {
    ...
    if (ts.canHaveModifiers(node)) {
        return (_a = ts.getModifiers(node)) !== null && _a !== void 0 ? _a : [];
    }
    return [];
}
function hasPublicMemberModifier(node) {
    return getModifiers(node).length > 0 && getModifiers(node).every(function (modifier) {
        return modifier.kind !== ts.SyntaxKind.ProtectedKeyword && modifier.kind !== ts.SyntaxKind.PrivateKeyword;
    });
}
function hasPublicConstructorModifier(node) {
    return getModifiers(node).length > 0 && getModifiers(node).some(function (modifier) {
...
function getInheritedProperties(modelTypeDeclaration, genericTypes) {
    ...
    heritageClauses.forEach(function (clause) {
        ...
        clause.types.forEach(function (t) {
            ...
            if (!type) {
                throw new Error("No type found for ".concat(t.expression.getText()));
            }
