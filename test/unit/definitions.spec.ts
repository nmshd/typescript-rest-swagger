...
describe('Definition generation', () => {
  ...
  describe('SecureEndpoint', () => {
    it('should apply controller security to request', async () => {
      ...
      expect(await expression.evaluate(spec)).toStrictEqual([{ 'access_token': ['ROLE_1', 'ROLE_2'] }]);
    });

    it('method security should override controller security', async () => {
      const expression = jsonata('paths."/secure".post.security');
      expect(await expression.evaluate(spec)).toStrictEqual([{ 'user_email': [] }]);
    });
  });
  ...
  describe('SuperSecureEndpoint', () => {
    it('should apply two controller securities to request', async () => {
      ...
      expect(await expression.evaluate(spec)).toStrictEqual([{ 'default': ['access_token'] }, { 'default': ['user_email'] }, { 'default': [] }]);
    });
  });
  ...
  describe('SpecGenerator', () => {
    it('should be able to generate open api 3.0 outputs', async () => {
      ...
      const expression = jsonata('paths."/supersecure".get.security');
      expect(await expression.evaluate(openapi)).toStrictEqual([{ 'default': ['access_token'] }, { 'default': ['user_email'] }, { 'default': [] }]);
