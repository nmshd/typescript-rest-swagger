...
(async function () {
    ...
    const metadata = new MetadataGenerator(["./test/data/apidebug.ts"], "./test/tsconfig.json").generate();
    const spec = new SpecGenerator(metadata, getDefaultOptions()).getOpenApiSpec();
    const specDeRef = (await swaggerParser.dereference(cloneDeep(spec) as any)) as unknown as Swagger.Spec;
