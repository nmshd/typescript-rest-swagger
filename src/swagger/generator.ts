...
export class SpecGenerator {
    ...
    public getSwaggerSpec() {
        ...
        spec.security = this.config.securityDefinitions ? this.config.securityDefinitions : [];
    ...
    private getSwaggerType(type: Type): Schema | undefined {
        ...
        if (unionType.types && unionType.types.length > 0) {
            let map = unionType.types.map((t) => this.getSwaggerType(t)).filter((t) => t !== undefined) as Schema[];
