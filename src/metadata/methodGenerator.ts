...
export class MethodGenerator extends EndpointGenerator<ts.MethodDeclaration> {
  ...
  public generate(): Method {
    ...
    if (bodyDecorator && bodyDecorator.length > 1) {
      throw new Error(
        `Only one Body decorator allowed in '${this.getCurrentLocation()}' method.`
      );
    }
    ...
    this.debugger(
      "Generated Metadata for method %s: %j",
      this.getCurrentLocation(),
      methodMetadata
    );
  ...
  private buildParameters(bodyDecorator: DecoratorData) {
    this.debugger(
      "Processing method %s parameters.",
      this.getCurrentLocation()
    );
    ...
    if (bodyParameters.length > 1) {
      debugger;
      throw new Error(
        `Only one body parameter allowed in '${this.getCurrentLocation()}' method.`
      );
    }
    ...
    this.debugger(
      "Parameters list for method %s: %j.",
      this.getCurrentLocation(),
      parameters
    );
    return parameters;
  ...
  private processMethodDecorators() {
    ...
    this.debugger(
      "Processing method %s decorators.",
      this.getCurrentLocation()
    );

    const pathDecorators = getDecorators(
      this.node,
      (decorator) => decorator.text === "Path"
    );
    ...
    if (pathDecorators) {
      ...
      this.path = pathDecorator
        ? `/${normalizePath(pathDecorator.arguments[0])}`
        : "";
    } else {
      this.path = "";
    }
  ...
  private mergeResponses(
    ...
    const index = responses.findIndex(
      (resp) => resp.status === defaultResponse.status
    );
