...
export function resolveType(typeNode?: ts.TypeNode, genericTypeMap?: Map<String, ts.TypeNode>): Type {
    ...
    const symbol = MetadataGenerator.current.typeChecker.getSymbolAtLocation(typeNameNode);

    let originalDeclarationFileName = sourceFile.fileName;
    if (symbol) {
        originalDeclarationFileName = getOriginalSourceFile(symbol) ?? sourceFile.fileName;
    }

    if (refType && refType?.originalFileName !== originalDeclarationFileName) {
        throw new Error(
            `reference type ${fullRefTypeName} with same name but different properties. Please use different names for different types.`
        );
    } else if (refType) {
        return refType;
    }
  ...
  `;
    ...
    if (type.isUnion()) {
        let unionType = {
            types: (
                (originalStatement.getType().compilerType.aliasSymbol?.declarations?.[0] as ts.TypeAliasDeclaration)
    ...
    const originalType = MetadataGenerator.current.typeChecker.getTypeAtLocation(typeReference);
    const typeArgumentsMap: Record<string, any> = {};
    if (typeArguments?.length ?? 0 > 0) {
        const typeParameter = (originalType.symbol.declarations?.[0] as ts.SignatureDeclarationBase).typeParameters;
    ...
    const properties = typeProperties
        .map((property): Property | undefined => {
            const declaration = property.getDeclarations()[0].compilerNode as ts.IndexSignatureDeclaration;
            if (declaration.kind) {
...
function getEnumerateType(typeNameNode: ts.EntityName): EnumerateType | undefined {
    let enumDeclaration = MetadataGenerator.current.typeChecker.getSymbolAtLocation(typeNameNode)
...
function getModelTypeProperties(node: any, genericTypes?: Array<ts.TypeNode>): Array<Property> {
    ...
    if (classConstructor && classConstructor.parameters) {
        properties = properties.concat(
            classConstructor.parameters.filter((parameter) => hasPublicConstructorModifier(parameter)) as any
        );
    }
    ...
    if (clauses) {
        ...
        if (filteredClauses.length > 0) {
            ...
            if (clause.types && clause.types.length > 0) {
                let type = MetadataGenerator.current.typeChecker.getSymbolAtLocation(clause.types[0].expression)
...
function buildGenericTypeMap(node: UsableDeclaration, typeArguments?: ReadonlyArray<ts.TypeNode>) {
...
function getOriginalSourceFile(symbol: ts.Symbol) {
    if (symbol && symbol?.declarations?.[0].kind === ts.SyntaxKind.ImportSpecifier) {
        return MetadataGenerator.current.typeChecker.getAliasedSymbol(symbol).getDeclarations()?.[0].getSourceFile()
            .fileName;
    }
