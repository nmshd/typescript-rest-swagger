...
export abstract class EndpointGenerator<T extends ts.Node> {
    ...
    protected getResponses(): Array<ResponseType> {
        ...
        if (!(tsMorphNode instanceof ClassDeclaration) && !(tsMorphNode instanceof MethodDeclaration)) {
            throw new Error(
                `Node at position ${
                    this.node.pos
        ...
        return tsMorphNode
            ...
            .map((decorator) => {
                ...
                if (args[1] && Node.isLiteralLike(args[1])) {
                    description = args[1].getLiteralText();
                }
                if (args[2]) {
