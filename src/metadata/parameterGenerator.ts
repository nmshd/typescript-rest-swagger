...
export class ParameterGenerator {
    ...
    private getCurrentLocation() {
        ...
        const controllerId = ((this.parameter.parent as ts.MethodDeclaration).parent as ts.ClassDeclaration)
    ...
    isRequired(parameter: ts.ParameterDeclaration): boolean {
        let isUndefinedUnion =
            ...
            (parameter.type as ts.UnionTypeNode).types.some((t: any) => t.kind === ts.SyntaxKind.UndefinedKeyword);
    ...
    private supportPathDataType(parameterType: Type) {
        return ["string", "integer", "long", "float", "double", "date", "datetime", "buffer", "boolean", "enum"].find(
            (t) => t === parameterType.typeName
        );
    }
